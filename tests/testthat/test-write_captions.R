...
#     dat |>
#       dplyr::filter(
#         label == "recruitment_deviations" |
#           label == "log_recruitment_deviations",
#         module_name == "SPAWN_RECRUIT" |
#           module_name == "t.series",!is.na(year),
#         is.na(fleet) | length(unique(fleet)) <= 1,
#         is.na(sex) | length(unique(sex)) <= 1,
#         is.na(area) | length(unique(area)) <= 1,
#         is.na(growth_pattern) |
#           length(unique(growth_pattern)) <= 1,!year %in% year_exclusions
#       ) |> # SS3 and BAM target module names
#       dplyr::mutate(estimate = as.numeric(estimate), year = as.numeric(year)) |>
